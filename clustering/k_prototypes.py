...
def wrap(data, iterations=1, max_num_clusters=20, algorithm='KPrototypes', random_pick=1, base=[], thresh_1=1, thresh_2=0.6,
         lr=0, backelim=False, stop=0.3, cruelty=1, preelim = False, corr_thresh = 0.7, variance_thresh = 0.01, significance = 0.001, dip = False):
    base_columns = []
    best_features = {}
    properties = {}
    if base:
            ...
            if base:
                ...
                if algorithm == 'KPrototypes':
                    if base_data.select_dtypes(['int64','int32','object','int']).shape[1] == base_data.shape[1]:
                        ...
                        best_method = 'KModes'
                        base_dip_result = dip_test(base_data)
                ...
                distributions = np.repeat(float('{:,.2f}'.format(base_distribution)), len(base_columns)).tolist()
                model_scores = np.repeat(float('{:,.3f}'.format(base_model_score)), len(base_columns)).tolist()
            ...
            while stopper == 0:
                ...
                while decider == 0:
                    try:
                        ...
                        dip_result = dip_test(std)
                        importance, verdict = xgb_validation(std, algorithm, base, clusters[-1])
                        if ((verdict == 'reject') | (dip_result < significance)) & (~dip):
                            model_score = -1
                            distribution = 1
                            best_method = algorithm
                            best_number_of_clusters = "Doesn't matter"
                            ...
                            if algorithm == 'KPrototypes':
                                if std.select_dtypes(['int64', 'int32', 'object', 'int']).shape[1] == std.shape[1]:
                                    ...
                                    n_clusters_k, model_score_k = mm.metrics()
                                    pretendents = mm.get_pretendents_
                                    model = KPrototypes_model(std, n_clusters_k)
                                    best_method = 'KPrototypes'
                            else:
                        ...
                        if (model_score >= np.max(np.array([base_model_score, score_thresh]))) & (
                            ...
                            base_dip_result = dip_result
                            base_columns = np.append(base_columns, new_column)
                            distributions = np.append(distributions, float('{:,.2f}'.format(distribution)))
                            model_scores = np.append(float('{:,.3f}'.format(model_score)), model_score)
                            ...
                            rejected_scores = np.append(rejected_scores, float('{:,.3f}'.format(model_score)))
                            rejected_distributions = np.append(rejected_distributions, float('{:,.2f}'.format(distribution)))
                            ...
                            counter += 1
                            if (model_score < 0) | (distribution > distr_thresh + 1):
                                for i in range(3):
                            ...
                            print(Fore.RED + str(counter) + '. ' + new_column[
                                ...
                                float('{:,.2f}'.format(distribution))) + '  |  Score: ' + str(
                                float('{:,.3f}'.format(model_score))) + '  |  ' + best_method + ': ' + str(best_number_of_clusters)
                                  + '  |  ' + str(importance) + ' : ' + str(dip_result))
                            pass
                        ...
                        if rejected_df[rejected_df['Distribution'] < distr_thresh].shape[0]:
                            ...
                            model_scores = np.append(float('{:,.3f}'.format(model_score)), base_model_score)
                            clusters = np.append(clusters, int(float(
...
class models_and_metrics:
    ...
    def metrics(self):
        if self.dip:
            ...
            weight = weights_for_gower(self.std)
            dist = gower_matrix(self.std.values, weight=weight)
            model_score = dip(dist, num_bins = 30)
        else:
